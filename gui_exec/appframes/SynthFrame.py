...
class SynthFrame(AppFrame):
    ...
    def run_command_synth_preview(self,init_type='Aliyun'):
        ...
        执行试听命令
        """
        command = 'RplGenCore.exe --Modules speech_synthesizer --PreviewOnly --Init {IN} --AccessKey {AK} --AccessKeySecret {AS} --Appkey {AP} --Azurekey {AZ} --ServRegion {SR} --Language zh '
    ...
    def run_command_synth(self):
        ...
        执行语音合成命令
        """
        command = 'RplGenCore.exe --Modules speech_synthesizer --LogFile {lg} --MediaObjDefine {md} --CharacterTable {ct} --OutputPath {of} --AccessKey {AK} --AccessKeySecret {AS} --Appkey {AP} --Azurekey {AZ} --ServRegion {SR} --Language zh '
